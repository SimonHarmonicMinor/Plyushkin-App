...
public class ExpenseNoteCategoryUseCase {
    ...
    @WriteTransactional
    public void addChildCategory(AddChildCommand command) throws AddChildException {
        repository.lockByWalletId(command.walletId());
        ExpenseNoteCategory root = repository.findByWalletIdAndNumber(command.walletId(), command.rootCategoryNumber())
                .orElseThrow(() -> new AddChildException.RootCategoryNotFound("Cannot find root category by WalletId %s and number %s".formatted(command.walletId(), command.rootCategoryNumber())));
        ExpenseNoteCategory child = repository.findByWalletIdAndNumber(command.walletId(), command.childCategoryNumber())
                .orElseThrow(() -> new AddChildException.ChildCategoryNotFound("Cannot find child category by WalletId %s and number %s".formatted(command.walletId(), command.rootCategoryNumber())));
        try {
            root.addChildCategory(child);
        } catch (AddChildCategoryException e) {
            switch (e) {
                case AddChildCategoryException.MismatchedWalletId err ->
                        throw new AddChildException.MismatchedWalletId("Mismatched WalletId " + command.walletId(), err);
                case AddChildCategoryException.ChildEqualsToRoot err -> throw new AddChildException.ChildEqualsToRoot("Child equals to root", err);
            }
        }
    ...
    @WriteTransactional
    public void changeParentCategory(ChangeParentCommand command) throws ChangeParentException {
        repository.lockByWalletId(command.walletId());
        ExpenseNoteCategory root = repository.findByWalletIdAndNumber(command.walletId(), command.rootCategoryNumber())
                .orElseThrow(() -> new ChangeParentException.RootNotFound("Cannot find root category by WalletId %s and number %s".formatted(command.walletId(), command.rootCategoryNumber())));
        ExpenseNoteCategory newParent =
                command.parentCategoryNumber() == null
                        ...
                        repository.findByWalletIdAndNumber(command.walletId(), command.parentCategoryNumber())
                                .orElseThrow(() -> new ChangeParentException.ParentNotFound("Cannot find child category by WalletId %s and number %s".formatted(command.walletId(), command.parentCategoryNumber())));
        try {
            root.changeParent(newParent);
        } catch (ChangeParentCategoryException e) {
            switch (e) {
                case ChangeParentCategoryException.MismatchedWalletId err ->
                        throw new ChangeParentException.MismatchedWalletId("Mismatched WalletId " + command.walletId(), err);
                case ChangeParentCategoryException.ParentEqualsToRoot err -> throw new ChangeParentException.ParentEqualsToRoot("Parent equals to root", err);
            }
        }
        repository.save(root);
    }
